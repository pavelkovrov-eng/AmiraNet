...
- **Guard every numeric engine entry point against non-finite input, and throw rather than fall back.** Any exported function taking a `number` that feeds stored state — theta, durations, time budgets, counts, difficulties — must reject `NaN` and `Infinity` with `Number.isFinite` and throw an Error naming the offending parameter and value. `Math.min`/`Math.max` do not filter `NaN`, so a clamp is not a guard. This was found in Task 4, where one `NaN` permanently poisoned the ability estimate and made `score >= 134` evaluate `false` forever, silently. A default-value fallback is not acceptable: it reintroduces the exact failure mode — a plausible-looking number that is wrong. Every guard needs a test per entry point.
